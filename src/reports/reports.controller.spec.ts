  ...
  describe("generate", () => {
    describe("yearly.csv", () => {
        it("generates the report correctly", async () => {
            ...
            expect(result).toEqual({ message: 'starting' });
            
            // Check that the yearly.csv file was created
            const yearlyFile = path.join(outDir, 'yearly.csv');
            const yearlyExists = await fs.access(yearlyFile).then(() => true).catch(() => false);
    ...
    describe("accounts.csv", () => {
        it("generates the report correctly", async () => {
            ...
            const accountsExists = await fs.access(accountsFile).then(() => true).catch(() => false);
    ...
    describe("fs.csv", () => {
        it("generates the report correctly", async () => {
            ...
            const fsExists = await fs.access(fsFile).then(() => true).catch(() => false);
            ...
            expect(fsContent).toContain('Assets = Liabilities + Equity, 200.00 = 450.00');
